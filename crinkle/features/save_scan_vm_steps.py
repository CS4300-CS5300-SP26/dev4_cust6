...
def step_stub_ai_grading(context):
    context.ai_patch = patch('cards.views.analyze_card_with_gemini', return_value=MOCK_GRADE_RESULT)
...
def step_save_graded_scan(context):
    context.save_response = context.client.get(reverse('cards:save_report'), follow=True)
...
def step_saved_scan_belongs_to_collection(context):
    ...
    assert CardCollection.objects.filter(user=context.user, cards=context.saved_card).exists()
