...
class Command(BaseCommand):  # pragma: no cover
    ...
    def handle(self, *args, **options):
        if not API_KEY:
            self.stdout.write(
                self.style.ERROR("POKEMON_PRICE_API_KEY not set")
            )
            return
        ...
        for card_name in CARDS:
            ...
            try:
                ...
                if not data.get("data"):
                    self.stdout.write(
                        f"  No results for {card_name}, skipping"
                    )
                    continue
                ...
                self.stdout.write(
                    self.style.ERROR(f"  Error fetching {card_name}: {e}")
                )
                continue

        self.stdout.write(
            self.style.SUCCESS(f"Loaded {loaded} pricing records")
        )
